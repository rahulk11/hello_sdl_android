...
public class SdlService extends Service implements IProxyListenerALM {
    ...
    private static final String DEV_MACHINE_IP_ADDRESS = "192.168.137.151";
...
//				sendInteractionChoiceSet();
            }
        });
        sendRpcRequest(command);
    }

    private int uniqueChoiceID = 345;
    private final int uniqueIntChoiceSetID = 456;

    private void sendInteractionChoiceSet() {
        ...
        choiceSet.setOnRPCResponseListener(new OnRPCResponseListener() {
            ...
            public void onResponse(int correlationId, RPCResponse response) {
                if (((CreateInteractionChoiceSetResponse) response).getSuccess()) {
                    Log.i("SdlService", "The request was successful, now send the SDLPerformInteraction RPC");
                } else {
                    Log.i("SdlService", "The request was unsuccessful");
                }
            }
        });
